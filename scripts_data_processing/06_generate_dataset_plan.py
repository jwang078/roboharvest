...
from data_processing.utils.cv_utils import get_gripper_width, get_pruner_z
from data_processing.utils.pose_utils import pose_to_mat, mat_to_pose
from data_processing.utils.interpolation_utils import (
...
def main(input, output, tx_slam_tag,
    ...
    with ExifToolHelper() as et:
        for video_dir in video_dirs:            
            ...
            hilight_csv_path = video_dir.joinpath('gopro_hilight.csv')
            if hilight_csv_path.is_file():
                try: 
                    hilights = pd.read_csv(hilight_csv_path, header=None)
                    hilights = hilights[0].tolist()
                except: # The csv file might be empty, i.e. no hilights
                    hilights = []
            hilights_timestamp = [start_timestamp + x for x in hilights]
    ...
    for demo_idx, demo_data in enumerate(demo_data_list):
        ...
        for s, is_valid in zip(segment_slices, segment_type):
            ...
            for cam_idx, row in demo_video_meta_df.iterrows():
                ...
                hilights = row['hilights']

            # Label each timestamp as a task_idx based on hilights
            task_idx = np.digitize(demo_timestamps[start:end], hilights, right=False).astype(np.int32)[:, None]
            # Label each observation with progress within the task: 0.0 for the first observation, 1.0 for the last
            unique_values, indices, counts = np.unique(task_idx, return_inverse=True, return_counts=True)
            progress = np.zeros_like(task_idx, dtype=np.float32)
            for uv, count in zip(unique_values, counts):
                progress[task_idx==uv] = np.linspace(0, 1, count)
            all_plans.append({
    ...
    print("n_dropped_cameras", dropped_camera_count)
